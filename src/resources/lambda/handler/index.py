...
# Set Log Group name from env var, and use the same log stream name for the AWS Lambda function
...
def put_log(records, *sequence_token):
    # Put log event, handle error to get sequence token, returns up to date sequence token.
    ...
    # The only place the sequence token is returned in the error is in the message, we have to parse out the token from the message.
        ...
        try:
            ...
            put_log_events_response = logs.put_log_events(
                **put_log_events_kwargs)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(e)
        print('Error getting key {key} from bucket {bucket}.')
        raise e
            ...
            if payload_size >= 1048576 or line_count >= 10000:
                try:
                    ...
                    if sequence_token is not None:
                        ...
                        print('This is the returned sequence token',
                              sequence_token)
